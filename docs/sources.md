...
Single owned-venue sources can opt in to `default_missing_location_to_owned_venue` when the feed itself is venue-scoped and individual event rows may omit venue fields. This is a review-stage missing-location default only: it does not change generic parser output, it does not apply to multi-venue feeds, and it does not override explicit per-event venue evidence.
...
- Hallamshire Hotel extracts the hidden public Google Calendar ICS URL from the official homepage, fetches the ICS payload, stores the raw ICS snapshot, and parses it with the Hallamshire ICS parser. Timed ICS rows follow the generic ICS semantics. Single-day all-day rows are kept only for explicit live-music title prefixes and receive an explicit inferred 19:30 Europe/London fallback start with blank end time; inferred starts do not publish through singleton or event-review auto-resolution paths. Trusted delegated detail URLs from Fatsoma, Leadmill, and WeGotTickets can be carried for enrichment/display, but they are excluded from source identity fallback so the calendar UID remains the source identity. The source is explicitly configured to default missing per-event venue evidence to `hallamshire-hotel` at review stage because the official feed is venue-scoped. Explicit `LOCATION` evidence is still interpreted normally and can block authority when it points elsewhere.
- The Washington extracts the official embedded FullCalendar Google Calendar configuration from the official events page and calendar frame, then fetches the corresponding Google Calendar API JSON. The live official calendar currently omits per-event `location` fields, so this source is explicitly configured to default missing venue evidence to `the-washington` at review stage for the known official Washington calendar ID only. Location-less events from any other Google Calendar ID are skipped, and explicit non-Washington venue evidence is still rejected.
...
- Hallamshire Hotel uses a configured hidden-calendar ICS-link extractor and a custom Hallamshire ICS parser for venue-scoped all-day fallback and delegated detail URL handling
...
Each persisted review candidate keeps the staged venue slug plus source-derived venue evidence used to interpret it: `venue_text` and `venue_location_raw`. For ICS sources, `venue_text` stays cleaned for display, while `venue_location_raw` preserves the unfolded raw `LOCATION` text so later venue parsing can decode ICS escapes before applying the normal comma/newline split; the fetched ICS payload remains raw in the snapshot. If a single owned-venue source opts in to `default_missing_location_to_owned_venue`, review staging may fill the venue slug from the configured owned venue only when both parser venue evidence fields are blank. Candidates can also carry optional room evidence (`room_text` and room slug/name rows) when a source exposes room-level listings.
...
- Likely ingest shape: `mode: linked_ics`, Hallamshire ICS parser, new small ICS link extractor for the Hallamshire page. Reuse the generic venue handling if ICS `LOCATION` data is usable; otherwise add a venue normalizer.
- Authority note: this can be an owned-venue authoritative source if the feed only lists Hallamshire events.
- Implementation status: implemented as `config/sources/08-hallamshire-hotel.yaml` with `mode: linked_ics`, `ics_parser: hallamshire_hotel`, `owned_venue_slug: hallamshire-hotel`, and `default_missing_location_to_owned_venue: true`. The hidden-calendar extractor prefers public Google Calendar ICS URLs before applying its limit. The parser does not fabricate venue evidence for missing ICS `LOCATION`; review staging applies the configured owned-venue slug only when `LOCATION` is absent. The Hallamshire parser keeps single-day all-day live-music rows with an explicit inferred 19:30 Europe/London fallback, skips multi-day and non-music all-day rows, and disables delegated detail URLs as source identity keys.
...
- Source contract: The Washington's live official Google Calendar API feed currently omits per-event `location` fields. Requiring per-event venue evidence would make the official source produce no candidates. The implementation therefore admits missing-location rows only from the known official embedded Washington calendar ID and leaves raw venue evidence blank; review staging applies `default_missing_location_to_owned_venue: true` for the source. Location-less events from any other Google Calendar ID remain skipped.
