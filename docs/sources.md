...
Each persisted review candidate keeps the staged venue slug plus source-derived venue evidence used to interpret it: `venue_text` and `venue_location_raw`. For ICS sources, `venue_text` stays cleaned for display, while `venue_location_raw` preserves the unfolded raw `LOCATION` text so later venue parsing can distinguish escaped punctuation and newline sequences from separator heuristics; the fetched ICS payload remains raw in the snapshot.
