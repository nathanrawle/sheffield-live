...
The admin UI exposes a landing page, event-review queue/history/detail pages, import history, provisional venue queue/detail pages, provisional room queue/detail pages, genre configuration, and per-run snapshot metadata when the backing store implements those read paths. Retired `/admin/legacy-review*` routes return 404 after admin auth.
