...
func autoResolveEventReviewClusterCanonicalExactMatchTx(ctx context.Context, tx interface {
	...
	snapshot, err := marshalEventReviewResolutionSnapshot(cluster, seedstore.EventReviewResolutionStatusResolved, "", nil, nil, &applied, nil, nil, nil, nil, nil, now)
	if err != nil {
		return nil, err
	}
...
func autoResolveEventReviewClusterUnanimousDuplicateTx(ctx context.Context, tx interface {
	...
	snapshot, err := marshalEventReviewResolutionSnapshot(cluster, seedstore.EventReviewResolutionStatusResolved, "", nil, nil, &appliedSnapshot, nil, nil, nil, nil, nil, now)
	if err != nil {
		return nil, err
	}
	if _, err := insertEventReviewResolutionTx(ctx, tx, cluster.ID, seedstore.EventReviewResolutionStatusResolved, snapshot, "", now); err != nil {
